...
bool	create_iol(t_io **io, t_io_t io_t)
{
	...
	if (g_neobash.prs_state)
		return (false);
	while (g_neobash.cur_tok && is_io())
	{
		io_t = get_type(g_neobash.cur_tok->type);
		ft_skip_tok();
		if (!g_neobash.cur_tok || g_neobash.cur_tok->type != WRD)
			return (set_state(1), false);
		new = create_io_node(g_neobash.cur_tok->value, io_t);
		if (!new)
